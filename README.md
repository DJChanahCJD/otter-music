...
- [ ] 聚合结果排序：多源合并后优先展示本地与最新可用数据，规则可测试。
- [ ] 错误自动上报：前端全局错误采集并写入服务端日志（保留 7 天）。
- [ ] 背景色测试，从 cache 中获取所有已缓存的专辑封面，分析颜色分布，确定合适的背景色，通过一个单独的测试页面展示。
- [ ] 全屏歌词切换时，样式存在跳动（从小变大，分行...）

### P1 下一步优化（体验）
- [ ] 专辑时间轴 Header：专辑页改为横向时间轴，并将滚动位置持久化到 Session。
- [ ] 缓存分级治理：按歌词/封面/API 统计缓存，并支持手动清理与自动 LRU。
- [ ] 播客入口迁移：按既有方案将入口迁移到更直观位置，减少查找成本。
- [ ] Otter 呼吸动效：全屏播放时封面外圈呼吸动画（类似网易云的孤独星球动效），暂停时静止，保留简洁风格。
