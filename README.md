...
- [ ] 修复网易云QR登录失效问题
- [ ] 支持网易云官方搜索（拦截请求，走自己的后端）
- [ ] _netease 歌曲做歌手、专辑跳转，直接复用歌单页面组件
- [ ] 在歌单市场添加（我的）分类，加载用户自己的网易云歌单
- [ ] 在歌单市场（全部）里添加个人推荐歌单（如果登录了网易云）
- [ ] 网易云歌单更多按钮那里添加分享按钮
- [ ] 「我的」页面下的歌单列表显示 cover
- [ ] 解锁完整版不能单纯看排名，必须歌名和歌手相同才匹配成功
- [ ] 全局手势是适配的，但是经典手势会存在遮挡，播放栏也有可能遮挡底部tab，考虑样式解决还是允许用户拖动改变y轴
- [ ] 添加逻辑：把个人歌单中的付费版本，点击匹配后直接改为免费版本，而不仅仅是下一首播放
- [ ] 在设置页面添加自动匹配功能按钮开关，即加载歌曲时如果需要付费，则自动匹配为免费音源。
