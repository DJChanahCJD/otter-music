...
* [ ] **全栈单体化**：整合前后端至同一仓库，利用 CF Pages + Functions 统一部署维护
* [ ] **智能回收站**：逻辑实现改为删除 7 天后自动清理
* [ ] **专辑编年史**：重构专辑页 Header 为横向滑动时间轴，并持久化滑动位置至 Session
* [ ] **状态同步修复**：解决 Web 与 Native 媒体控制栏（MediaSession）同步问题
* [ ] **分级缓存清理**：实现按类型（歌词、封面、API）分类统计并手动/自动LRU清理
