...
- [ ] 歌曲播放重试优化（指数退让？增大重试或切歌次数？熔断冷启动？）最重要的getSongUrl可优先走负载均衡(非local、podcast、_netease)
- [ ] 播放时断网后有时无法正常切换到有本地下载的歌曲
- [ ] 从专辑页回来能否保持专辑drawer？
- [ ] 评估是否引入 howler.js 作为音频播放库
- [ ] 去除 Tauri 相关代码，通过 PWA 实现桌面端功能，做基本布局适配（如md:, lg:等）
- [ ] 我的歌单页面没有做header吸附；「我的」页面也需要做header吸附
- [ ] 图标需要添加背景
