...
interface PodcastState {
  ...
  addRssSource: (name: string, rssUrl: string) => string;
  updateRssSource: (id: string, data: Pick<PodcastRssSource, "name" | "rssUrl">) => void;
...
export const usePodcastStore = create<PodcastState>()(
  persist(
    (set) => ({
      ...
      addRssSource: (name, rssUrl) => {
        const id = uuidv4();
        set((state) => ({
          rssSources: [
            markUpdate({ id, name, rssUrl, is_deleted: false }),
            ...state.rssSources,
          ],
        }));
        return id;
